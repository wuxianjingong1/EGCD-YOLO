...
# from ultralytics.nn.modules.odconv import C2f_OD,ODConv2d
...
def parse_model(d, ch, verbose=True):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        ...
        if m in {
            ...
            args = [c2, c2]
        elif m in {FocusedLinearAttention,deformable_LKA_Attention,LSKA,DAttentionBaseline,SimAM,ECA,GAM,CoordAtt,CBAM,SEAttention,ContextGuidedBlock_Down,OREPA,MultiDilatelocalAttention}:
            c2 = ch[f]
            args=[c2,*args]
            # ----------------MobileViT-------------------
        elif m is MV2Block:
...
def yaml_model_load(path):
    ...
    unified_path = re.sub(r"(\d+)([nslmx])(.+)?$", r"\1\3", str(path))  # i.e. yolov8x.yaml -> train.yaml
