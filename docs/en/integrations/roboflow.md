...
keywords: Roboflow, YOLO11, data labeling, computer vision, model training, model deployment, datasets management, automated image annotation, AI tools
---
...
dataset = project.version(VERSION).download("train")

project.version(dataset.version).deploy(model_type="train", model_path=f"{HOME}/runs/detect/train/")
```
